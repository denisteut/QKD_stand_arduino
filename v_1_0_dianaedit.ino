...
void Init(){
  ...
  ToLineal();
  SetBaseAngle();
  while(Stand.NoiseLevel1 + Stand.NoiseLevel2 > 12000){
        SetLaserPower(Stand.MaxLaserPower - 1);
        delay(100);
        Stand.MaxLaserPower -= 1;
        GetSignalsLevels();
 }
  return;
}
...
void ToLineal(){
 ...
 /* if(Stand.NoiseLevel1 + Stand.NoiseLevel2 > 10000){
    int max_signal = 0;
    int new_delta = 2000;
    max_signal = Stand.NoiseLevel1;
    if (max_signal < Stand.NoiseLevel2){
      max_signal = Stand.NoiseLevel2;
      while((max_signal-Stand.NoiseLevel2) < new_delta){
        SetLaserPower(Stand.MaxLaserPower - 1);
        Stand.MaxLaserPower -= 1;
        GetSignalsLevels();
      }
    }else{
      while((max_signal-Stand.NoiseLevel1) < new_delta){
        SetLaserPower(Stand.MaxLaserPower - 1);
        Stand.MaxLaserPower -= 1;
        GetSignalsLevels();
      }
